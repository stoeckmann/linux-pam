	   ...
	   const char *source, struct dirent *dent)
{
   ...
   if (S_ISLNK(st.st_mode))
   {
	 ...
	 if (symlinkat(pointed, parent->fd, dent->d_name) != 0)
	    ...
	    if (retval != PAM_SUCCESS)
	       pam_syslog(NULL, LOG_DEBUG,
			  "unable to create link %s: %m", dent->d_name);
#ifndef PATH_MAX
	    free(pointed);
#endif
	    goto go_out;
	 ...
	 {
	    pam_syslog(NULL, LOG_DEBUG,
		       "unable to change perms on link %s: %m", dent->d_name);
#ifndef PATH_MAX
	    free(pointed);
#endif
			...
			O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW, 0600)) < 0)
   {
      ...
      if (retval != PAM_SUCCESS)
         pam_syslog(NULL, LOG_DEBUG,
		    "unable to open dest file %s: %m", dent->d_name);
      goto go_out;
   }
   ...
   {
      pam_syslog(NULL, LOG_DEBUG,
		 "unable to change perms on copy %s: %m", dent->d_name);
      retval = PAM_PERM_DENIED;
      goto go_out;
   }
	       ...
	       mode_t dir_mode, const char *source, const char *dest)
{
 ...
 go_out:
   ...
   {
      pam_syslog(NULL, LOG_DEBUG,
		 "unable to change perms on directory %s: %m", base.path);
      retval = PAM_PERM_DENIED;
   }
		      ...
		      const char *_skeldir, const char *_homedir)
{
   ...
   if (cp == NULL)
   {
      pam_syslog(NULL, LOG_ERR,
		 "Relative user home directory: %s", _homedir);
      return PAM_SESSION_ERR;
   }

   *cp = '\0';
   retval = dir_spec_open(&base,
			  cp == NULL ? "." : cp == _homedir ? "/" : _homedir);
   if (retval < 0)
   {
      pam_syslog(NULL, LOG_DEBUG,
